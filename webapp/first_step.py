...
def setup_json_of_basic_known_words():
...
def make_dictionary_of_any_word_with_frequency():
    word_freq = {}
...
def gather_movie_vocabulary(path, name_of_movie):   
    regex_words =r"[a-zA-Z]+'*[a-zA-Z]+"
    regex_sentences =r".*"  #FIXME
    ...
    for subtitle in subtitle_generator:
        ...
        for sentence in sentences:
            ...
            for word in words:
                ...
                word_data['source'] = name_of_movie 
...
def main():
    known_words = setup_json_of_basic_known_words()

    path = input("Give me a path to a subtitle file, please.")
    name_of_movie = input("What is the name of the movie?")
    movie_vocabulary = gather_movie_vocabulary(path, name_of_movie)
